...
public:
    ...
    MazeSolver(enum SceneType scene_number, bool is_forward_direction) : Node("maze_solver"), scene_number_(scene_number), is_forward_direction_(is_forward_direction) {
    ...
    timer_1_ = this->create_wall_timer(
        2000ms, std::bind(&MazeSolver::timer1_callback, this));//nothing about 1 sec
...
bool pid_simulate_rotating(double x_goal, double y_goal, double tolerance, double angle_tolerance){
  ...
  void execute() {
    ...
    for(auto it2 = std::next(ref_points->begin(),1); it2 != ref_points->end(); it2++){        
        ...
        RCLCPP_INFO(this->get_logger(), "start move ref_points w%d  (%f,%f), current_yaw_rad %f: %s",
        w_name, xg, yg, this->current_yaw_rad_, result_pid_rotating.c_str());
        ...
        ling.linear.y = 0;
        this->move_robot(ling);
        RCLCPP_INFO(this->get_logger(), "end move ref_points w%d  (%f,%f), current_yaw_rad %f: %s, elasped time %ld",
